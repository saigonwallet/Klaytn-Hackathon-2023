...
type BonsaiRwaProps = {
  item: Nft;
  buyRwa: (token: number, value: number) => Promise<void>;
...
const BonsaiRwa: FunctionComponent<BonsaiRwaProps> = ({item, buyRwa}) => {
  return (
    <>
    ...
    <div className="flex-1 bg-white p-6 flex flex-col justify-between">
      ...
      <div className="flex-1">
        <div className="flex justify-between items-center">
          <div className="flex items-center mt-2">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-gray-700 group-hover:text-gray-900">Artisan</p>
            ...
            <div className="flex flex-col px-4 pt-4">
              ...
              <dd className="order-1 text-xl font-extrabold text-indigo-600">
                <div className="flex justify-center items-center">
                  {item.price}
                  <img className="h-6" src="/images/small-klaytn.webp" alt="klaytn icon"/>
                </div>
              </dd>
            </div>
      ...
      <div>
          <button
            onClick={() => {
              buyRwa(item.bonsaiId, item.price);
            }}
            type="button"
            className="disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none disabled:cursor-not-allowed mr-2 inline-flex items-center px-4 py-2 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
          >
           {(item.creator === Account.name) ? "Unlist Nft" : "Buy"}
          </button>
          <button
            type="button"
